...
# 定义文件夹路径
dnase_dir = "/path/of/DNase/"
hic_dir = "/path/of/NegativeNpy_Fin/"
output_dir = "/path/of/DNase-HiC/Negative/"
# 检查输出目录是否存在，不存在则创建
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# 获取所有的 DNase 和 Hi-C 文件列表
dnase_files = sorted([f for f in os.listdir(dnase_dir) if f.endswith("_0.npy")])
...
for dnase_file in dnase_files:
    ...
    # 找到对应染色体号的 Hi-C 文件
    hic_file = f"KR_5kb_matrix_{chrom}_negative.npy"
    ...
    if hic_file in hic_files:
        ...
        if dnase_data.shape[0] == hic_matrices.shape[0]:
            ...
            # 保存为 .npz 文件
            output_file = os.path.join(output_dir, f"Dnase_Hic_{chrom}_0.npz")
