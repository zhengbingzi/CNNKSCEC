...
def getbigwig(file, chrome, start, end):
    ...
    if start >= end:
        return np.zeros((21, 5000))  # 返回全零的矩阵
        # return np.zeros((21, 10000))  # 返回全零的矩阵
        # return np.zeros((21, 25000)) 
...
def generateATAC(files_center_point, files_dnase, resou):
    ...
    with open(files_center_point) as f:
        ...
        for line in f:
            ...
            # window_y = window_y.reshape(21, 25000)
...
if __name__ == '__main__':
    ...
    for n in range(len(a)):                                               
        files_center_point = '/path/of/PositiveTxt/positive_chr'+str(a[n])+'.txt'
        files_dnase = "/path/of/merged.bigWig"#基于DNase-seq的bigWig文件
        resou = 5000
        all_matrix1 = generateATAC(files_center_point, files_dnase, resou)
        all_matrix_array = np.array(all_matrix1)
        np.save('/path/of/DNase/dnase_chr' + str(a[n]) + '_1.npy', all_matrix_array)
