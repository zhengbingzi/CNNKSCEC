...
class DeepCNN(nn.Module):
    ...
    def forward(self, data):
        ...
        b, _, _, _ = data.size()
        # encode process
        out1 = self.pool1(self.relu(self.conv1(data)))
        out1_ = self.relu(self.conv1_(data))
        out_e1 = self.dropout(self.eca1(self.scconv1((out1_ + out1) / 2) + (out1_ + out1) / 2))
        skip1 = out_e1

        out1 = self.pool2(self.relu(self.conv2(out_e1)))
        out1_ = self.relu(self.conv2_(out_e1))
        out_e2 = self.dropout(self.eca2(self.scconv2((out1_ + out1) / 2) + (out1_ + out1) / 2))
        skip2 = out_e2
        
        out1 = self.pool3(self.relu(self.conv3(out_e2)))
        kd_out1 = out1.view(b, -1)
        out1_ = self.relu(self.conv3_(out_e2))
        kd_out1_ = out1_.view(b, -1)
        
        out_e3 = self.dropout(self.eca3(self.scconv3((out1_+out1)/2)+ (out1_+out1)/2) )
        ...
        # classifier
        out2 = self.linear2(self.drop(self.relu(self.linear1(skip4.view(b, -1)))))
